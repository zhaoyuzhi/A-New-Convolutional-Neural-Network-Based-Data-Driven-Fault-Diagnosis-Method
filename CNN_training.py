...
IMAGESAVEURL_training = "E:/study/code/training"                  #your training saving image URL
IMAGESAVEURL_validation = "E:/study/code/validation"              #your validation saving image URL
...
x_data = np.zeros([160,64,64], dtype = np.float32)                #input training_data
y_data = np.ones([160,2], dtype = np.float32)                     #correct output training_data
x_test = np.zeros([64,64,64], dtype = np.float32)                 #input testing_data
y_test = np.ones([64,2], dtype = np.float32)                      #correct output testing_data

## handle training data
for i in range(160):
    #get 36*36 numpy matrix of a single image
    ...
    #get 0 & 1
    label = training_table.cell(i,1).value
...
m=tf.clip_by_value(a_fc2, 1e-10, 1.0)
cross_entropy = tf.reduce_mean(-tf.reduce_sum(y * tf.log(m), reduction_indices=[1]))
...
with tf.Session() as sess:
    ...
    z=0
    # train model
    for i in range(10):                                           #number of iterations:2000*1125=2250000, 36 millon images
        for m in range(10):                                       #training process using training data 18000 images
            ...
            sess.run(train_step, feed_dict = {x:train_xbatch, y:train_ybatch, keep_prob:0.5})
            z=z+1
            if z % 4 == 0:
                print('    step:',z)
    ...
    # test trained model
    for j in range(4):
        test_xbatch = x_test[(j*16):(j*16+16),:,:]               #test 100 data every batch, not including k*100+100
        test_ybatch = y_test[(j*16):(j*16+16),:]		           #test 100 data every batch, not including k*100+100
